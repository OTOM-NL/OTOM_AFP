...
function [Rot_Roller_axis,n_xyz]=Rot_Matrix_Finder_RollerTape_dome_Car(tv,th_y,R_cyl,z_cyl_end,Vec_tang,Dir)
...
% n_xyz=[tv(1)/a^2; tv(2)/b^2 ;tv(3)/c1^2];
n_xyz=n_xyz/norm(n_xyz);
...
% wind_angle=wind_angle- (.0001);  % to avoid numerical instability
...
% n_xyz=[tv(1)/a^2; tv(2)/b^2 ;(tv(3)-z_cyl_end)/c2^2];
n_xyz=n_xyz/norm(n_xyz);
...
% wind_angle=wind_angle- (.000001);  % to avoid numerical instability
...
%%
% winding path should be along the x-dir
...
%  end
   ...
   th_y=sign(cosd(theta_normal_cross_product))* theta_2Refconfig+Dir;
