...
function [kr1_active,beta_macro]=refl_Mandrel_plot3D_BRDF(R_cyl,xyz_int,laser_direction,z_cyl_end,fileID_Normals,fileID_Refs,BRDF_sub)

% BRDF_sub=[Fib_Rot,div,sig_T,sig_F,Amp,threshold]


%  Fib_Rot=BRDF_sub(1);


% beta_macro: angle between macro normal and incoming ray
% kr1_active (1:3,:) direction of reflectred from microfacet
% kr1_active (4,:) corresponding energy weight
...
%   Fib_Rot=00; % in degree
  

if ~isempty(x_int)
    
    
    % First calculating normals
    if R_cyl(1) ~=0  
        
        t=linspace(0,R_cyl(1)/5,2);
        
        
        
        if z_int >= 0 & z_int <=z_cyl_end
            if (abs(x_int^2+y_int^2 - R_cyl(1)^2) < 1e-2)   % Seems not necessary
                
                norm_n=norm([x_int y_int]);
                
                %normal in x-y plane- MACRO MODEL
                ...
                % Calculating Microsurface normals, location of
                % intersection, fiber orientation, normal macro model,
                % inoming ray direction
%                  [Rot_G2BRDF,n_xyz]=Rot_Matrix_Finder_local_BRDF(xyz_int,fib_or,[S_normal 0],laser_direction);
    [Rot_Roller_axis,beta_macro,S_normal_active,kr1_active]=Rot_Matrix_Finder_local_BRDF(R_cyl,z_cyl_end,xyz_int,BRDF_sub,S_normal ,laser_direction);

     [m,n]=size(S_normal_active);
        
                for ii=1:n
                normal_line_X(1,:)=S_normal_active(1,ii)*t+x_int;
                normal_line_Y (1,:)=S_normal_active(2,ii)*t+y_int;
                     normal_line_Z (1,:)=S_normal_active(3,ii)*t+z_int;
    ...
    Ref_line_X(1,:)=kr1_active(1,ii)*t+x_int;
    Ref_line_Y (1,:)=kr1_active(2,ii)*t+y_int;
    Ref_line_Z (1,:)=kr1_active(3,ii)*t+z_int;
