...
function thermal_domain_generator_UOT(th_y,W_tape,thick_T,R_tape,L_flat,deg_tape,...
    sui,~ ,~,thick_sub,No_dev,R_cyl,...
    ...
    UOT_pathfile,CV_mesh,Laser_head,Laser_ID,...
    text_status,Points_in_domain_all_sub,Points_in_domain_all_Tape,Tape_points_Data)


 

Video = VideoWriter('.\Temp_evoloution','MPEG-4');
% video.FrameRate = 10;
...
%   Graphic_profile=Graphic_chekbox(5);
%   Grpahic_contour=Graphic_chekbox(6) ;
%
%
%    Graphic_Profile_T=Graphic_chekbox (3);
%    Graphic_contour_T=Graphic_chekbox (4);
...
%  Laser_ID=[3,50,50, 50,50, 0.01,0.01];

Power_Actual=Laser_Power_generator (Laser_head_Ax,Laser_head_Ay,Laser_head_nx,Laser_head_ny,Laser_ID);
...
if Graphic_chekbox(1)
    ...
    st=strcat(cd,'/Supp_files/Postprocess_Parameter.txt');
fid13 = fopen(st);
...
Q_roller=zeros(steps,1);
Temp_roller_current=Temp_Right_T_Roller(2); %+ (4*40);

% Do not consider roller temperature !!
% for ss=1:steps
...
%        Mac
           st=strcat(cd,'/UOT_Roller_Q_Temp.txt');
fileID_Tecplot_Roller_Q_Temp = fopen(st,'w');
...
%        fileID_Tecplot_Roller_Q_Temp = fopen('.\UOT_Roller_Q_Temp.txt','w');
 ...
 if Graphic_chekbox(1)
                ...
                st=strcat(cd,'/Sub3D_Temp_RedBox.txt');
fileID_Temp_Red_Box_Sub = fopen(st,'w');

           st=strcat(cd,'/Tape_Temp_RedBox.txt');
fileID_Temp_Red_Box_Tape = fopen(st,'w');
...
Temp_roller_increase=Roller_temp_estimation(ro_roller,R_tape,W_R,cp_roller,time,Q_roller(ss));
...
%       disp max intensity
% Intensity_max(ss,1:2)=([max(E_points_4_adv_func/sub_ele_size),max(E_points_4_adv_func_T/Tape_ele_size)]);
    end
...
figure(59);
cla;
plot(linspace(-Lx,0,length(T_old(index_middle_long))),T_old(index_middle_long),'b');
hold on;
plot(linspace(-Lx_T,0,length(T_old_T(index_middle_long_T))),T_old_T(index_middle_long_T),'r');
legend ('Substrate','Tape','Location','northwest');
axis([-Lx_T 0 0 700]);
xlabel('Length [m]');
ylabel('Temperature [^{\circ}C]');

nip=(T_old(index_middle_long(end))+T_old_T(index_middle_long_T(end)))/2;
str=sprintf('Temp Nip= %6.2f °C, time step= %d \n  Temp Sub=%6.2f °C, Temp Tape=%6.2f °C',nip,ss,T_old(index_middle_long(end)),T_old_T(index_middle_long_T(end)) );
title(str);
% pause(0.1);
Mov(ss)=getframe(gcf);
%%
...
% fileID16= fopen(sprintf('.\T_Tape%d.txt',ss),'w');
