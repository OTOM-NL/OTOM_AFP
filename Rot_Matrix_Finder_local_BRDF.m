...
function [Rot_Roller_axis,beta_macro,S_normal_active_G,kr1_active_G]=Rot_Matrix_Finder_local_BRDF(R_cyl,z_cyl_end,xyz_int,BRDF_sub,n_Macro,laser_direction)
...
Fib_or=90+Fib_Rot; % repect to the tangent to the helical direction
...
[Rot_Roller_axis,phi_Fiber]=Rot_Matrix_Fiber_BRDF(xyz_int,Fib_or,R_cyl,z_cyl_end,Vec_Pr_v,n_Macro');
  ...
  % should be Rotated 
%  theta_normal_cross_product_Pr_v=acosd((vector_cross_Pr_v*n_Macro')/(norm(vector_cross_Pr_v)*norm(n_Macro)));

%   phi_0=atand(project_ray(2)/project_ray(1));
% =0-(sign(cosd(theta_normal_cross_product_Pr_v))*phi_0)+


 phi_Fiber=-sign(vector_cross_Pr_v(3))*acosd(-project_ray(1))+ 180;%0*90; %+Fib_Rot+0;
...
theta=beta_macro;  % got from macro calculation , Angle incidence respect to macro normal
