...
fid_acc = fopen('.\Supp_files\Account.txt','r');

if  fid_acc >0
    Acc_def = textscan(fid_acc,'%s','delimiter',',');
    Acc_def=char(Acc_def{:});
...
dlg_title = 'OTOM composite Account';
defaultans = {Acc_def};
answer = inputdlg(prompt,dlg_title,[1 50],defaultans);
% the preference file should save the data like account
% a = findall(handles);
...
if  ~isempty(answer)
    ...
    [Out,Access_level]=Security_OTOM(answer);
    
    
    fid_acc = fopen('.\Supp_files\Account.txt','w');
    
    if  fid_acc >0
        fprintf(fid_acc,'%s',answer);
    ...
    if Out ==0
        ...
        set( handles.Account, 'Enable', 'On');
        set( handles.Exit, 'Enable', 'On');
        set( handles.About, 'Enable', 'On');
        set( handles.Share, 'Enable', 'On');
        
         set( handles.Documents, 'Enable', 'On');
...
%% Set off button based on Access_level
% Access_level(2:12)=0;


 if Access_level(1)==0
   set( handles.BRDF, 'Enable', 'off'); 
 end
if Access_level(2)==0
     set( handles.Transient_analysis, 'Enable', 'off'); 
    
end
if Access_level(3)==0
    set( handles.Input_file_mode, 'Enable', 'off'); 
    
end
if Access_level(4)==0
     set( handles.Manufacturing_type, 'Enable', 'off'); 
    
end
if Access_level(5)==0
     set( handles.ADS_Client_File, 'Enable', 'off'); 
...
if Access_level(7)==0
     set( handles.Optical_UOT, 'Enable', 'off'); 
    
end
if Access_level(8)==0
     set( handles.Thermal_UOT, 'Enable', 'off'); 
    
    
   
end
if Access_level(9)==0
     set( handles.Measuring_Box_Definition, 'Enable', 'off'); 
    
end
if Access_level(10)==0
     set( handles.Free_winding_Sub, 'Enable', 'off'); 
    end


if Access_level(11)==0
     set( handles.Load, 'Enable', 'off'); 
     
    
end
if Access_level(12)==0
     set( handles.Save, 'Enable', 'off'); 
    
end
if Access_level(13)==0
     set( handles.Roller_def, 'Enable', 'off'); 
    
end
if Access_level(14)==0
     set( handles.Free_winding, 'Enable', 'off'); 
   
end

if Access_level(15)==0
     set( handles.Optimization, 'Enable', 'off'); 
...
if  ~isempty(answer)
    ...
    % Check weather the PC is registered
    answer=char(answer);
   
%     Out=Security_OTOM(answer);
     [Out,Access_level]=Security_OTOM(answer);
...
selection = questdlg('Do you want to register this PC by sending the PC names to OTOMcomposite?',...
    'Warning', ...
    'Yes','No','Yes');
...
if selection(1:2)=='Ye'
    ...
    % read whole sentences , 72
    % mac_all=status(160:end);
    ...
    % current_mac_temp=strsplit(mac_all,'\n');
    % whether it is a laptop or just a PC, networking card!
    
    if length(current_mac_temp)==3
        
        message=sprintf('%s <br> \r\n %s  <br> \r\n %s  <br> \r\n %s',PC_name , mac_all{1},mac_all{2}, CPU_name );
        
    else
        message=sprintf('%s  <br> \r\n %s  <br> \r\n %s',PC_name , mac_all{1}, CPU_name );
        
    end
    
      h2 = msgbox(message,'To OTOMcomposite.com');
    ...
    h2=warndlg('You did not send any information to OTOMcomposite.','!! Warning !!');
    %     [cdata,map] = imread('under_construction.png');
    % web('https://www.otomcomposite.com');
    %
    % h2=msgbox('Under Development!',...
