% clc;
% clear;

function [Main_path,thermal_points_display,Ps]=helical_3D_points_4subW(R,z_end,L,w_T,sui,No_dev_L,No_dev,Nip_Mov,tv)




if length(R)==3
   
    c1=R(2);
     c2=R(3);
else
       c1=R(1);
     c2=R(1);
%       R_cyl=R_cyl(1);
    
end

a=R(1);
b=R(1);




% h1=figure(1);
%   set(h1,'Visible','off');

R=R+R/200; % just for representation

phi=sui*(pi/180); %pi/2-alpha; 



%%
x0=tv(1);
y0=tv(2);
zp=tv(3);


%%
% for cylinder

th=L*cos(phi)/a; %in radian
H=L*sin(phi);

 theta0=atan2(x0,y0);

% No_dev_L=40;
t_L=linspace(theta0,theta0+th,No_dev_L);
x_L=0+a*sin(t_L);
y_L=0+a*cos(t_L);
z_L=zp+linspace(0,H,No_dev_L);

Main_path=[x_L;y_L;z_L];
%  plot3(x_L,y_L,z_L,'g.-');


%%
% for the cylidenrical part

% xp=x_L(kk);
% yp=y_L(kk)-R(1);
% zp=z_L(kk);

th_up=w_T*cos(phi+pi/2)/a; %in radian
H=w_T*sin(phi+pi/2);


t_up=linspace(theta0,theta0+th_up,No_dev);
x_up=0+a*sin(t_up);
y_up=0+a*cos(t_up);
z_up=zp+linspace(0,H,No_dev);

% plot3(x_up,y_up,z_up,'r.-');
%%
% xp=x_L(kk);
% yp=y_L(kk)-a;
% zp=z_L(kk);

th_low=w_T*cos(phi-pi/2)/a; %in radian
H=w_T*sin(phi-pi/2);

t_low=linspace(theta0,theta0+th_low,No_dev);
x_low=0+a*sin(t_low);
y_low=0+a*cos(t_low);
z_low=zp+linspace(0,H,No_dev);

% plot3(x_low,y_low,z_low,'g.-');

%Translation to the new position, Rotation around z





points=[x_up;y_up; z_up];
points2=[x_low;y_low; z_low];

No_row_nodes=(2*No_dev)-1;


thermal_points_Temp=zeros(3,No_dev_L*No_dev);

t_L2=linspace(0,th,No_dev_L);


starting_index=floor(No_dev_L*(Nip_Mov/abs(L))); % after this position , the nip-point will be defined

No=(No_dev_L-starting_index)*No_row_nodes;

for pp=1:No_dev_L
    
    tv=[0, 0 z_L(pp)-zp];
% tv=zeros(3,1);
     Rot=t_L2(pp);

    new_points=Transformation_Rot_z (tv,Rot,points);
      new_points2=Transformation_Rot_z (tv,Rot,points2);

      
thermal_points_Temp(1:3,(1:No_row_nodes) + (pp-1)*No_row_nodes )=[new_points2(1:3,end:-1:1),new_points(1:3,2:end)];

%     plot3(new_points(1,:),new_points(2,:),new_points(3,:),'r.-');
%      plot3(new_points2(1,:),new_points2(2,:),new_points2(3,:),'g.-');
    
end

thermal_points=thermal_points_Temp(1:3,1+((starting_index)*No_row_nodes):end);

% 1 index along the width is ignored, since we use width node from dome
thermal_points_display=thermal_points_Temp(1:3,1+((1+starting_index)*No_row_nodes):end);

% plot3(thermal_points(1,:),thermal_points(2,:),thermal_points(3,:),'g*')
% Collect boarder points

P1s=thermal_points(1:3,(1:No_row_nodes-1));
% P2s=thermal_points(1:3,No_row_nodes:No_row_nodes:No);
P2s=thermal_points(1:3,2*No_row_nodes:No_row_nodes:No);
P3s=thermal_points(1:3,No-1:-1:No-No_row_nodes+2);
% P4s=thermal_points(1:3,No-No_row_nodes+1:-No_row_nodes:No_row_nodes+1);
P4s=thermal_points(1:3,No-No_row_nodes+1:-No_row_nodes:No_row_nodes+0);

% Ps=[P1s, P2s,P3s, P4s];

Ps=[ P2s,P3s, P4s];
%  theta_all=on_out_surface (p,Ps)

% plot3(Ps(1,:),Ps(2,:),Ps(3,:),'y*')

