...
function [xyz_int]=Line_Mandrel_intersection_4subW(laser_source_P,laser_direction,R_cyl,z_cyl_end)
...
for ii=1:length(x_int)
    ...
    elseif z_int(ii) >= 0 & z_int(ii) <=z_cyl_end
        ...
        if abs(theta_Fn)<=90
            ...
            theta_V_AB_Line3D=acosd(V_AB'*Rxyz  );
...
if ~imag(t1)
    ...
    if (zp1 >=z0_E1 &&    zp1 <= z0_E1+c2);
        ...
        if abs(theta_Fn1)<=90
            ...
            if abs(real(theta_V_AB_Line3D))<=1e-1  % parallel
...
%    R_cyl(1) ==0 ;
   ...
   xyz_int=Placement_int (laser_source_P,Rxyz );
