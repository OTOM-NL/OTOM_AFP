...
function [xyz_int]=Line_cylinder_intersection_Roller(laser_source_P,laser_direction,R_cyl,z_cyl_end)
...
%%
% before it was  ==>> y0=Ly -(Ry/Rx)*Lx

% m=Ry/Rx;
% 
% syms x m R y0
% 
% f=@(x) x^2 -R^2 + (m*x + y0 )^2
...
if x_int
    ...
    if z_int<-z_cyl_end/2 | z_int > z_cyl_end/2
        %         warning('There is no intersection between line and surface!')
        x_int=[];
        y_int=[];
        z_int=[];
        
        % it means the line passes both sides
        %%
        %compute the intersection from the line
        %decide from which ends of the cylinder the ray leave the body
        
        x_int_new=([-z_cyl_end/2 z_cyl_end/2] -Lz)*(Rx/Rz)+Lx;
        y_int_new=([-z_cyl_end/2 z_cyl_end/2] -Lz)*(Ry/Rz)+Ly;
...
%         elseif z_int(2) > -z_cyl_end/2 & z_int(2) < z_cyl_end/2
...
%             z_int(1)=((Rz/Rx)*(x_int (1)-Lx))+Lz;
    ...
    elseif z_int>-z_cyl_end/2 & z_int < z_cyl_end/2
        ...
        if z_int(1) > -z_cyl_end/2 & z_int(1) < z_cyl_end/2
