...
function [h_T,Rot_Roller_axis]=Tape_plot_3D_ALL_objects_Dome_Car(N,W,R,L_flat,tv,th_y,thick_T,deg_T,W_R,theta_ind,R_cyl,z_cyl_end,H_indentation,Vec_tan,Dir)



if R_cyl (1) ~=0
%%
[Rot_Roller_axis,~]=Rot_Matrix_Finder_RollerTape_dome_Car(tv,th_y,R_cyl,z_cyl_end,Vec_tan',Dir);
...
% Roller_Def=n_xyz*H_indentation;


% Deformation along the normal direction
...
Temp2=Rot_Roller_axis*[xo_text;yo_text;zo_text];
xo_text=Temp2(1,:);
yo_text=Temp2(2,:);
zo_text=Temp2(3,:);
...
h_axis_local=plot3(xo_points, ...
       yo_points, ...
      zo_points , 'k--');
h_text=text(xo_text, yo_text, zo_text, ['+X';'+Y';'+Z']);
hold on;
% {h_axis_local,h_text}
%%
...
%% Modified for the indentation - vertical movement of the Roller-Tape
y=-y+R_thick-H_indentation;  % for tape
y_R=-y_R+R-H_indentation;    % for roller   I have some doubts....  !!!
...
Temp=Rot_Roller_axis*[x(1,i);y(1,i);z(1,i)];
Temp2=Rot_Roller_axis*[x(2,i);y(2,i);z(2,i)];
...
for i=1:size(x_R,2)
   Temp_R=Rot_Roller_axis*[x_R(1,i);y_R(1,i);z_R(1,i)];
Temp2_R=Rot_Roller_axis*[x_R(2,i);y_R(2,i);z_R(2,i)];
...
h_R1=fill3([X2 X],[Y2 Y],[Z2 Z],[Temp Temp],'FaceAlpha',0.8);

h_R0=surf(x_G_R,y_G_R,z_G_R,'FaceColor',[0.95 0.7 1],'LineStyle','None');
...
%     h_T= [h3  h_axis_local]; %for the flat and curved part of the tape

   h_T= {h3  , h_axis_local,h_text,h_R};
